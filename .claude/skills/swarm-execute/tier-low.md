...
> **Reviewer model**: every `worker-reviewer` launch this tier uses resolved `--reviewer` overlay value (tier=low default `sonnet`; `haiku` only via explicit user override). See `overlays.md` reviewer axis.
