...
Control model for each `worker-researcher` launched during Research phase. Sonnet 5 is the default floor for all research at every tier — narrow single-axis lookups and multi-axis synthesis alike. Haiku is never selected automatically; it runs only when the user passes `--researcher=haiku` explicitly, as a deliberate override.

| Value | Effect |
|---|---|
| `haiku` | `worker-researcher` with model=haiku. Explicit user override only — no automatic trigger. |
...
- high → `sonnet`
- max → `sonnet` (research=3 is synthesis)

**Context-cap guard**: if the user forces `--researcher=haiku`, Haiku 4.5's 200k context window is a hard constraint — if the research prompt projects to exceed 150k tokens (e.g., WebFetch on >5 sources, reading >5 large files), escalate to `sonnet` regardless of the override. See `.claude/artifacts/research_model_capability_matrix.md` for context-cap rationale.
