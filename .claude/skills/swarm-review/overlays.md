...
Evidence from `.claude/artifacts/research_model_capability_matrix.md`: Opus 4.7 leads Sonnet 4.6 by 8.0pp on SWE-bench Verified; gap biggest on multi-step agentic chains (adversarial breadth profile at tier=max).
...
Controls model for `worker-doc-reviewer` when it launches (Stage 2 at tier=high/max when doc triggers match).
...
**Codex model per tier** (passed to `codex-adversary` as `--model`): low (only if forced) → `terra` (`gpt-5.6-terra`; `luna`/`gpt-5.6-luna` only via explicit `--codex-model=luna` override), high → `terra` (`gpt-5.6-terra`), max → `sol` (`gpt-5.6-sol`). Override with `--codex-model=luna|terra|sol`. Policy: `workflow-swarm.md` "Cross-model model tiers".
...
| codex-model | terra (if forced; luna = explicit cheap override) | terra | sol |